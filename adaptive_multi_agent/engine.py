...
# 尝试加载共享复杂度评估器；不可用时保持 AMA 原有评估器作为 fallback
try:
...
class _SharedAwareAssessor:
    """包装共享复杂度评估器，对外保持 AMA 原有评估格式与接口。

    优先调用 ``ComplexityAssessor.assess()``，将统一格式转换为 AMA 原有格式；
    当共享评估不可用或调用失败时，回退到 ``TaskComplexityAssessor``。
    """

    def __init__(self):
        self._fallback = TaskComplexityAssessor()
        self._shared_class = ComplexityAssessor
    ...
    def assess(
        ...
        if external_assessment is not None:
            return self._fallback.assess(task_description, context, external_assessment)

        if self._shared_class is not None:
            try:
                shared = self._shared_class()
                unified = shared.assess(task_description, context=context)
                return self._to_ama_format(unified)
            except Exception as exc:
                logger.warning("共享复杂度评估失败，回退到 AMA 原有逻辑: %s", exc)

        return self._fallback.assess(task_description, context)

    @staticmethod
    def _to_ama_format(unified: Dict) -> Dict:
        """将 ``ComplexityAssessor`` 的统一格式转换为 AMA 原有格式。"""
        score_10 = float(unified.get("score_10", 3.0))
        features = unified.get("features", {})
        task_type = unified.get("task_type", "default")

        # 复用 AMA 评估器内部的 token 估算与模式推荐逻辑
        estimated_tokens = TaskComplexityAssessor()._estimate_tokens(features, score_10)
        recommended_mode = TaskComplexityAssessor()._preliminary_recommendation(
            score_10, task_type, features
        )

        return {
            "complexity_score": score_10,
            "task_type": task_type,
...
class AdaptiveMultiAgentEngine:
    ...
    def __init__(self, config: Optional[Dict] = None):
        self.assessor = _SharedAwareAssessor()
        self.config = {
        ...
        self.selector = ModeSelectionEngine(
            ...
            config={k: v for k, v in {
                "mode_rules": self.config.get("mode_rules"),
                "cold_start_prior": self.config.get("cold_start_prior"),
                "cost_map": self.config.get("cost_map"),
        ...
        self._human_input_mode = self.config.get("human_input_mode", "NEVER")
        self.skill_registry = get_skill_registry()
        self.current_workflow = None
        self._switch_cooldown: Dict[str, float] = {}
        self._switch_cooldown_seconds = float(self.config.get("switch_cooldown_seconds", 30.0))
