...
class SyntheticDatasetBuilder:
    ...
    MAX_SKILL_CHARS = 15000  # 截断阈值，防止 API 连接超时
    ...
    def generate(self, skill_text: str, num_cases: int = 15) -> EvalDataset:
        ...
        # 截断过长 skill text 防止 API 连接超时
        if len(skill_text) > self.MAX_SKILL_CHARS:
            logger.info(f"[SyntheticDatasetBuilder] skill text {len(skill_text)} chars, 截断至 {self.MAX_SKILL_CHARS}")
            skill_text = skill_text[:self.MAX_SKILL_CHARS]

        prompt = _BUILD_PROMPT.format(num_cases=num_cases, skill_text=skill_text)

        # 带重试的 API 调用（最多 3 次，指数退避）
        raw = None
        last_exc = None
        for attempt in range(3):
            try:
                ...
                if attempt < 2:
                    wait = 2 ** (attempt + 1)
                    logger.warning(f"[SyntheticDatasetBuilder] API 调用失败 (attempt {attempt+1}/3), {wait}s 后重试: {e}")
...
class BoundaryProbeBuilder:
    ...
    def generate(self, skill_text: str, num_cases: int = 10) -> list[EvalExample]:
        ...
        # 截断过长 skill text
        if len(skill_text) > 15000:
            skill_text = skill_text[:15000]
