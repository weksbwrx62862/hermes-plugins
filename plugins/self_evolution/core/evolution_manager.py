...
class EvolutionManager:
    ...
    def evolve_skill(
        ...
        # ── 阶段 5: CONSTRAINTS — Layer 3: 10维 AND 门控 ─
        logger.info(f"[evolve:{skill_name}] STAGE 5/8 CONSTRAINTS — 10-dimension AND gate")
        try:
            # baseline 检查：跳过 size_limit 和 growth_limit（进化目的之一就是压缩大技能）
            constraints = provider.validate_constraints(skill_text)
            # 放行 size_limit、growth_limit、silent_bypass_check、l1_compliance(长度) 的 baseline 检查失败
            # （size_limit: 进化目的之一就是压缩大技能）
            # （silent_bypass_check: 大文档中 "bypass" 一词常为误报）
            # （l1_compliance length: 与 size_limit 冗余）
            baseline_hard_failures = [
                f for f in constraints.failures
                if not f.startswith("size_limit")
                and not f.startswith("growth_limit")
                and not f.startswith("silent_bypass_check")
                and "content too long" not in f
            ]
            if baseline_hard_failures:
                logger.warning(
                    f"[evolve:{skill_name}] baseline constraint FAILED: {baseline_hard_failures}"
                )
                return EvolutionResult(
                    target_name=skill_name, phase=phase,
                    ...
                    error=f"Baseline failed constraints: {', '.join(baseline_hard_failures)}",
                )
            if constraints.failures:
                logger.info(f"[evolve:{skill_name}] baseline relaxed: size_limit/growth_limit failures ignored for baseline")
            logger.info(f"[evolve:{skill_name}] 10-dimension AND gate: PASSED (baseline-relaxed) ✓")
        except Exception as exc:
        ...
        try:
            evolved_text, evolved_score, iters_used, audit_report = provider.optimize(
                skill_text, dataset, iterations=iterations, skill_name=skill_name
            )
        ...
        # ── 阶段 8: DEPLOY（带版本快照 + Diff Review）─────
        logger.info(f"[evolve:{skill_name}] STAGE 8/8 DEPLOY — with snapshot management")

        # 8.1 初始化快照管理器
        snapshot_mgr = None
        try:
            from self_evolution.core.snapshot_manager import (
                SnapshotManager, format_diff_report
            )
            snapshot_mgr = SnapshotManager(skill_path.parent)
        except Exception as exc:
            logger.debug(f"[evolve:{skill_name}] SnapshotManager 初始化失败: {exc}")

        # 8.2 保存基线版本快照（如果还没有）
        if snapshot_mgr:
            try:
                baseline_version = snapshot_mgr.get_next_version(mode="static")
                # 只在没有基线时保存
                if not snapshot_mgr.get_latest():
                    snapshot_mgr.save_snapshot(
                        skill_text=skill_text,
                        version="v1",
                        trigger="auto",
                        mode="baseline",
                        reason="Original version before optimization",
                        baseline_score=baseline_score,
                        evolved_score=baseline_score,
                        skill_name=skill_name,
                    )
            except Exception as exc:
                logger.debug(f"[evolve:{skill_name}] baseline snapshot failed: {exc}")
        ...
        if improvement > 0:
            ...
            if snapshot_mgr:
                try:
                    evolved_version = snapshot_mgr.get_next_version(mode="static")
                    snapshot_mgr.save_snapshot(
                        skill_text=evolved_text,
                        version=evolved_version,
                        trigger="auto",
                        mode="static",
                        reason=f"Evolution improved by {improvement:+.3f}",
                        baseline_score=baseline_score,
                    ...
                    # 生成 Diff 报告
                    diff_result = snapshot_mgr.diff_versions("v1", evolved_version)
                    diff_report = format_diff_report(diff_result)
                    logger.info(
                        f"[evolve:{skill_name}] diff: +{diff_result.added_lines}/"
                        f"-{diff_result.removed_lines} lines, "
                        f"sections: {diff_result.changed_sections}"
                    )
                except Exception as exc:
                    logger.debug(f"[evolve:{skill_name}] snapshot/diff failed: {exc}")
            ...
            if auto_deploy:
                try:
                    ...
                    if deployed:
                        # 标记快照为已接受
                        if snapshot_mgr and snapshot_mgr.get_latest():
                            snapshot_mgr.accept_version(snapshot_mgr.get_latest())
                ...
                logger.info(
                    f"[evolve:{skill_name}] improvement={improvement:+.3f}, "
                    f"snapshot saved but NOT deployed (auto_deploy=False). "
                    f"Use self_evo_approve to deploy, self_evo_reject to discard."
                )
        else:
        ...
        return EvolutionResult(
            ...
            cross_model_score=cross_model_score,
            audit_report=audit_report,
            snapshot_version=snapshot_mgr.get_latest() if snapshot_mgr else "",
            diff_report=diff_report or "",
        )
    ...
    def _load_target(self, skill_name: str, phase: EvolutionPhase) -> tuple[Optional[str], Optional[Path]]:
        ...
        match phase:
            case EvolutionPhase.SKILL:
                ...
                # 递归搜索子目录（支持 skills/category/skill-name/ 结构）
                for base in [targets_dir / "skills", Path.home() / ".hermes" / "skills"]:
                    if not base.exists():
                        continue
                    for found in base.rglob(f"{skill_name}/SKILL.md"):
                        if found.is_file():
                            return found.read_text(), found
            case _:
                pass
        return None, None
