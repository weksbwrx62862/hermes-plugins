...
示例:
  ...
  %(prog)s --rollback my-skill
    ...
    # 版本快照命令（借鉴 Skill-insight）
    parser.add_argument(
        "--snapshot-list", metavar="SKILL_DIR",
        help="列出指定 skill 的所有版本快照",
    )
    parser.add_argument(
        "--snapshot-diff", nargs=2, metavar=("OLD_VER", "NEW_VER"),
        help="比较两个版本的差异（需配合 --snapshot-list 的 SKILL_DIR）",
    )
    parser.add_argument(
        "--snapshot-revert", metavar="VERSION",
        help="回退到指定版本（需配合 --snapshot-list 的 SKILL_DIR）",
    )
    parser.add_argument(
        "--snapshot-accept", metavar="VERSION",
        help="接受指定版本（小版本→主版本晋升）",
    )
    ...
    parser.add_argument(
        ...
        help="用于评估和变异的模型 (默认: mimo-v2.5-pro)",
    )
    parser.add_argument(
    ...
    if args.snapshot_list:
        from self_evolution.core.snapshot_manager import SnapshotManager
        sm = SnapshotManager(args.snapshot_list)
        versions = sm.list_versions()
        if not versions:
            print("📭 暂无版本快照")
            return 0
        print(f"{'版本':<10} {'触发':<8} {'模式':<10} {'改进':<10} {'已接受':<8} {'原因'}")
        print("-" * 80)
        for v in versions:
            accepted = "✅" if v.get("accepted") else "❌"
            improvement = v.get("improvement", 0)
            imp_str = f"{improvement:+.3f}" if improvement else "—"
            print(f"{v['version']:<10} {v.get('trigger','?'):<8} {v.get('mode','?'):<10} "
                  f"{imp_str:<10} {accepted:<8} {v.get('reason','')[:40]}")
        return 0

    if args.snapshot_diff:
        from self_evolution.core.snapshot_manager import SnapshotManager, format_diff_report
        # 需要从环境或参数获取 skill_dir
        skill_dir = args.target_dir or str(Path.home() / ".hermes")
        sm = SnapshotManager(skill_dir)
        old_ver, new_ver = args.snapshot_diff
        try:
            diff = sm.diff_versions(old_ver, new_ver)
    ...
    if args.snapshot_revert:
        from self_evolution.core.snapshot_manager import SnapshotManager
        skill_dir = args.target_dir or str(Path.home() / ".hermes")
        sm = SnapshotManager(skill_dir)
        if sm.revert_to(args.snapshot_revert):
            print(f"✅ 已回退到 {args.snapshot_revert}")
        else:
            print(f"❌ 回退失败")
            return 1
        return 0
    ...
    if args.snapshot_accept:
        ...
        skill_dir = args.target_dir or str(Path.home() / ".hermes")
        sm = SnapshotManager(skill_dir)
        if sm.accept_version(args.snapshot_accept):
            print(f"✅ 已接受 {args.snapshot_accept}")
        else:
            print(f"❌ 接受失败：版本不存在")
            return 1
        return 0
    ...
    if args.evolve:
        ...
        print("=" * 60)
        print(f" 自进化引擎 v1.0")
        print(f" 8阶段 Loop | 3层评测 | 10维 AND 门控")
        print("=" * 60)
        print()
