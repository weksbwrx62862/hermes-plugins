...
六种模式:
  ...
  - parallel_fusion: 并行融合（高复杂度 + 多视角分析任务）
"""
...
from .handlers import (
    ...
    handle_ama_trajectories,
    handle_on_session_end,
    handle_on_session_start,
    handle_post_tool_call,
    handle_ama_resume,
)
from .persistence import migrate_legacy_trajectories
...
_TOOLS = (
    ...
    ("ama_trajectories", AMA_TOOL_SCHEMAS.get("ama_trajectories", {}), handle_ama_trajectories, "📋"),
    ("ama_grade", AMA_TOOL_SCHEMAS.get("ama_grade", {}), handle_ama_grade, "⚖️"),
    ("ama_skills", AMA_TOOL_SCHEMAS.get("ama_skills", {}), handle_ama_skills, "🧠"),
    ("ama_workflow", AMA_TOOL_SCHEMAS.get("ama_workflow", {}), handle_ama_workflow, "📋"),
    ("ama_resume", AMA_TOOL_SCHEMAS.get("ama_resume", {}), handle_ama_resume, "▶️"),
)
...
def register(ctx) -> None:
    ...
    if ctx is None or not hasattr(ctx, "register_tool"):
        ...
        logging.getLogger(__name__).warning(
            "adaptive_multi_agent: ctx 无效或缺少 register_tool 方法，跳过注册"
        )
        return
    ...
    try:
        ...
        # 安全地执行历史 trajectory 数据迁移；失败不影响后续工具注册
        try:
            migrate_legacy_trajectories()
        except Exception as migrate_err:
            import logging
            logging.getLogger(__name__).exception(
                "adaptive_multi_agent: 历史 trajectory 数据迁移失败: %s", migrate_err
            )
        ...
        import logging
        logging.getLogger(__name__).exception(
            "adaptive_multi_agent: register 失败: %s", e
        )
