...
class TaskState(Enum):
    """任务级状态机（借鉴 Adversarial Agent Team 的状态流转）。

    单任务内的状态流转：
    INIT → PLANNING → EXECUTING → REVIEWING → (REVISING → EXECUTING → REVIEWING)* → DONE/FAILED
    """
    INIT = "init"               # 初始化
    PLANNING = "planning"       # 规划中：拆解任务、确定策略
    EXECUTING = "executing"     # 执行中：子智能体正在工作
    REVIEWING = "reviewing"     # 审核中：验证产出质量
    REVISING = "revising"       # 修订中：根据反馈改进
    AGGREGATING = "aggregating" # 汇总中：合并子任务结果
    DONE = "done"               # 完成
    FAILED = "failed"           # 失败

    @property
    def cn(self) -> str:
        return _TASK_STATE_CN.get(self, self.value)
...
_TASK_STATE_CN: dict = {
    ...
    TaskState.REVISING: "修订中",
    TaskState.AGGREGATING: "汇总中",
    TaskState.DONE: "已完成",
    TaskState.FAILED: "失败",
}
...
_TASK_STATE_TRANSITIONS: dict = {
    TaskState.INIT: {TaskState.PLANNING, TaskState.FAILED},
    TaskState.PLANNING: {TaskState.EXECUTING, TaskState.FAILED},
    TaskState.EXECUTING: {TaskState.REVIEWING, TaskState.AGGREGATING, TaskState.FAILED, TaskState.DONE},
    TaskState.REVIEWING: {TaskState.DONE, TaskState.REVISING, TaskState.FAILED},
    TaskState.REVISING: {TaskState.EXECUTING, TaskState.FAILED},
    TaskState.AGGREGATING: {TaskState.DONE, TaskState.FAILED},
    TaskState.DONE: set(),
...
class TaskStateMachine:
    ...
    追踪单个任务的状态流转，确保状态转换合法。
    支持回调通知（用于进度上报）。
    """

    def __init__(self, task_id: str, on_transition=None):
        self.task_id = task_id
        self._state = TaskState.INIT
        self._history: list = [(TaskState.INIT, time.time())]
        self._on_transition = on_transition  # callback(from_state, to_state, task_id)
    ...
    def transition(self, new_state: TaskState) -> bool:
        ...
        """
        with self._lock:
            allowed = _TASK_STATE_TRANSITIONS.get(self._state, set())
            ...
            if self._on_transition:
                try:
                    self._on_transition(old_state, new_state, self.task_id)
                except Exception:
                    pass
    ...
    def elapsed(self) -> float:
        """从初始化到现在的时间（秒）。"""
        with self._lock:
            return self._elapsed_internal()

    def _elapsed_internal(self) -> float:
        """内部版本，不加锁（调用方需已持有锁）。"""
        if self._history:
            return time.time() - self._history[0][1]
        return 0.0
    ...
    def summary(self) -> dict:
        ...
        with self._lock:
            ...
            for i in range(1, len(self._history)):
                prev_state, prev_time = self._history[i - 1]
                curr_state, curr_time = self._history[i]
                transitions.append({
            ...
            return {
                ...
                "elapsed": self._elapsed_internal(),
...
_MODE_CN: Dict[AgentMode, str] = {
    ...
    AgentMode.PARALLEL_FUSION: "并行融合",
}
...
_MODE_PRESETS: Dict[AgentMode, SubagentConfig] = {
    ...
    AgentMode.PARALLEL_FUSION: SubagentConfig(
        name="parallel_fusion",
        description="并行融合模式：多个模型并行回答同一问题，由裁决模型综合各答案生成最优结果",
        system_prompt=(
            "你是一个并行融合代理。你的职责是："
            "1. 将同一问题并行发送给多个模型获取独立回答；"
            "2. 收集所有模型的回答，分析共识与分歧；"
            "3. 识别各回答中的独特见解和潜在错误；"
            "4. 综合所有回答的优点，生成一个更高质量的最终答案。"
            "重点：不是简单投票，而是深度综合和交叉验证。"
        ),
        timeout_seconds=600,
        max_turns=5,
        max_retries=2,
...
# ── 独立的 Generator / Verifier 配置（借鉴 Adversarial Agent Team）──
# Generator 和 Verifier 使用不同的角色描述，确保真正的对抗性

_GENERATOR_CONFIG = SubagentConfig(
    name="generator",
    description="生成代理：专注于高质量内容产出",
    system_prompt=(
        "你是一个专业的生成代理。你的唯一职责是根据任务要求生成高质量的结果。\n\n"
        "核心原则：\n"
        "1. 专注于产出，不要自我审查\n"
        "2. 按照任务要求完整、准确地完成工作\n"
        "3. 如果有反馈，请严格按照反馈要求改进\n"
        "4. 输出清晰、结构化、可直接使用\n\n"
        "你不需要验证自己的产出——验证由独立的审核代理负责。"
    ),
    timeout_seconds=300,
    max_turns=10,
    max_retries=3,
...
_VERIFIER_CONFIG = SubagentConfig(
    name="verifier",
    description="验证代理：独立审核产出质量",
    system_prompt=(
        "你是一个独立的验证代理。你的唯一职责是严格审核其他代理的产出。\n\n"
        "你与生成代理是完全独立的——你不知道也不关心它的工作过程，只看最终产出。\n\n"
        "审核标准：\n"
        "1. 正确性：产出是否准确完成了任务要求？\n"
        "2. 完整性：是否遗漏了关键要素？\n"
        "3. 一致性：逻辑是否自洽？\n"
        ...
        "- 如果不通过，列出所有问题并给出改进建议\n\n"
        "请以 JSON 格式返回：\n"
        '{"passed": true/false, "issues": ["问题1", "问题2"], "feedback": "整体评价和改进建议"}'
    ),
    timeout_seconds=180,
    max_turns=5,
    max_retries=2,
...
class RetryPolicy:
    # 可重试的错误类别（包含 AMA 统一分类与内部重试分类）
    RETRYABLE_ERRORS = {
        "rate_limit", "network", "context_overflow", "json_parse_error",
        "internal_error", ErrorCategory.timeout.value, ErrorCategory.unknown.value,
    }
    ...
    max_retries_by_category: Dict[str, int] = {
        ...
        "json_parse_error": 2,
        "internal_error": 1,
        ErrorCategory.timeout.value: 2,
        ErrorCategory.unknown.value: 1,
    }
    ...
    def get_retry_hint(self, error_category: str) -> str:
        hints = {
            ...
            ErrorCategory.timeout.value: "请简化输出，减少不必要的详细描述",
            ErrorCategory.subagent_failure.value: "子代理执行失败，请检查输入与工具可用性",
        }
        return hints.get(error_category, "")

    def classify_error(self, exception: Exception) -> str:
        """将异常映射为内部重试分类；超时与未知错误使用 AMA 统一分类。"""
...
class CircuitBreaker:
    ...
    def record_failure(self, error_category: Optional[str] = None) -> None:
        """记录一次失败；error_category 用于可观测性，暂不影响熔断状态。"""
        with self._lock:
            self._failure_count += 1
            self._last_failure_time = time.time()
            if error_category:
                logger.debug("熔断器记录失败: category=%s", error_category)
...
class TerminationCondition:
    ...
    def __or__(self, other: "TerminationCondition") -> "OrTermination":
        # 展平嵌套的 OrTermination：(a|b) | (c|d) → OrTermination(a,b,c,d)
        left = self.conditions if isinstance(self, OrTermination) else [self]
        right = other.conditions if isinstance(other, OrTermination) else [other]
        return OrTermination(*left, *right)
...
class MaxMessagesTermination(TerminationCondition):
    ...
    def check(self, context: Dict) -> bool:
        return context.get("message_count", 0) >= self.max_messages

    def __repr__(self):
        return f"MaxMessages({self.max_messages})"
...
class TokenBudgetTermination(TerminationCondition):
    ...
    def check(self, context: Dict) -> bool:
        return context.get("tokens_used", 0) >= self.max_tokens
...
class SuccessRateTermination(TerminationCondition):
    """成功率过低时终止（防止无意义重试）"""

    def __init__(self, min_rate: float = 0.2, min_trials: int = 3):
        self.min_rate = min_rate
        self.min_trials = min_trials

    def check(self, context: Dict) -> bool:
        trials = context.get("trials", 0)
        successes = context.get("successes", 0)
        if trials < self.min_trials:
            return False
        return (successes / trials) < self.min_rate

    def __repr__(self):
        return f"SuccessRate({self.min_rate})"
...
class TerminationGuard:
    ...
    用法：
        guard = TerminationGuard(
            MaxMessagesTermination(10) | TokenBudgetTermination(5000) | TimeoutTermination(120)
        )
        if guard.should_terminate(context):
            ...
    """

    def __init__(self, condition: Optional[TerminationCondition] = None):
