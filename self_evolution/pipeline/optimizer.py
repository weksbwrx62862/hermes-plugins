...
from self_evolution.core.reflection_store import get_reflection_store, EvolutionReflection
...
_CROSSOVER_PROMPT = """你是 AI agent 技能优化专家。将下面两个版本的技能合并为一个更优版本。
...
class SkillOptimizer(SkillOptimizerBase):
    ...
    def optimize(
        ...
        _ref_store = get_reflection_store()
        ...
        for iteration in range(1, iterations + 1):
            ...
            candidates = self._diversify(current_text, combined_feedback, temperature=adjusted_temp, skill_name=skill_name)
        ...
        # 保存本次进化反思到持久化存储
        if skill_name and reflections:
            for ref in reflections:
                if ref.should_mutate:
                    _ref_store.add_reflection(skill_name, EvolutionReflection(
                        timestamp=time.time(),
                        failure_type=ref.reflection_type,
                        weak_dimension=self._detect_weak_dim(ref.feedback),
                        feedback_summary=ref.feedback[:200],
                        mutation_strategy="unknown",
                        score_before=current_score,
                        score_after=best_score,
                        lesson=ref.feedback[:200],
                    ))
    ...
    def _crossover(self, version_a: str, version_b: str) -> str:
        """交叉算子 — 合并两个版本的最佳段落。"""
        prompt = _CROSSOVER_PROMPT.format(version_a=version_a[:3000], version_b=version_b[:3000])
        try:
            ...
            return result.strip() if result.strip() else version_a
        except Exception as e:
            logger.debug("[optimizer._crossover] failed: %s", e)
            return version_a

    def _diversify(self, current_text: str, feedback: str, temperature: float = 0.7, skill_name: str = "") -> list[DiversifyResult]:
        """策略多样化变异 — 生成 K 个不同候选。"""
        ...
        reflections_text = ""
        if skill_name:
            reflections_text = get_reflection_store().get_lessons_prompt(skill_name)
    ...
    def _mutate(self, current_text: str, feedback: str,
                iteration: int = 1, total_iterations: int = 5, reflections_prompt: str = "") -> str:
        ...
        prompt = _MUTATE_PROMPT.format(
            current_text=current_text,
            feedback=feedback[:2000],
            reflections_prompt=reflections_prompt,
        )
...
class GEPAOptimizer(SkillOptimizerBase):
    ...
    def __init__(self, client: OpenAI, model: str = "gpt-4o-mini", use_llm: bool = True, skill_name: str = ""):
        self.client = client
        self.model = model
        self.use_llm = use_llm
    ...
    def optimize(
        ...
        for iteration in range(1, iterations + 1):
            ...
            mutated_text = self._mutate(current_text, combined_feedback, temperature, reflections_prompt=reflections_text)
        ...
        if skill_name:
            ...
            _ref_store.add_reflection(skill_name, EvolutionReflection(
                ...
                weak_dimension="unknown",
                feedback_summary=combined_feedback[:200] if combined_feedback else "",
                mutation_strategy="gepa",
                score_before=current_score,
                score_after=best_score,
                lesson=f"GEPA {iteration}轮 {current_score:.3f}→{best_score:.3f}",
            ))
    ...
    def _mutate(self, current_text, feedback, temperature, reflections_prompt=""):
        prompt = _MUTATE_PROMPT.format(current_text=current_text, feedback=feedback[:2000], reflections_prompt=reflections_prompt)
        response = self.client.chat.completions.create(
            model=self.model,
            messages=[{"role": "user", "content": prompt}],
            temperature=temperature,
