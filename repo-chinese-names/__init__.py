...
def register(ctx=None):
    ...
    # 注册仓库中文名查询工具
    ctx.register_tool(
        name="repo_chinese_name",
        toolset="repo-chinese-names",
        schema={
            "type": "object",
            "properties": {
                "repo_name": {"type": "string", "description": "仓库名"},
                "fuzzy": {"type": "boolean", "description": "是否启用模糊匹配", "default": False},
            },
            "required": ["repo_name"],
        },
        handler=lambda args, **kw: get_chinese_name(
            args.get("repo_name", ""), fuzzy=args.get("fuzzy", False)
        ),
        description="查询 GitHub 仓库的中文名称映射",
    )

    # 注册仓库列表格式化工具
    ctx.register_tool(
        name="repo_list_format",
        toolset="repo-chinese-names",
        schema={
            "type": "object",
            "properties": {
                "repos": {"type": "array", "description": "仓库列表"},
                "show_category": {"type": "boolean", "description": "是否显示分类", "default": False},
            },
            "required": ["repos"],
        },
        handler=lambda args, **kw: format_repo_list(
            args.get("repos", []),
            show_category=args.get("show_category", False),
        ),
        description="格式化仓库列表，显示中文名和分类",
    )
    ...
    ctx.register_tool(
        ...
        handler=lambda args, **kw: json.dumps(
            search_repos(args.get("query", "")), ensure_ascii=False
        ),
        description="搜索仓库（按中文名或英文名）",
    )
