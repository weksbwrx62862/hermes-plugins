...
_MIMO_KEY_ENV_VARS = ["MIMO_API_KEY_2", "MIMO_API_KEY_3", "MIMO_API_KEY", "MIMO_API_KEY_4"]
...
# MiMo 优先 key 数量：前 N 个 key 为主用 key，轮转只在主用 key 间进行
# 主用 key 全部不健康时才降级到备选 key
_MIMO_PREFERRED_KEY_COUNT = 2  # Key2, Key3 为主用
...
_DEFAULT_SCORING: Dict[str, Any] = {
    "off_peak_mimo_bonus": 40,  # MiMo 全天优先（从25提高到40确保胜过免费NVIDIA）
...
def _build_mimo_key_to_base_url() -> Dict[str, str]:
    ...
    default_base = _load_config().get("providers", {}).get("mimo", {}).get("base_url", "")
...
def _pick_mimo_key() -> str:
    return _pick_key_for_provider("mimo", "mimo", _MIMO_KEY_ENV_VARS, preferred_count=_MIMO_PREFERRED_KEY_COUNT)
